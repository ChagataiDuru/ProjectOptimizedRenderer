...
glm::mat4 Camera::perspectiveReverseZ(float fovY_, float aspect, float n, float f) {
    // Reverse-Z: depth range [1, 0] — precision concentrated near the far plane.
    // Differs from glm::perspective only in result[2][2] and result[3][2] signs.
...
void Camera::setPerspective(float fovYDegrees, float aspectRatio_, float nearZ_, float farZ_) {
    fovY        = fovYDegrees;
